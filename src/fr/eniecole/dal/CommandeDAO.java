...
public class CommandeDAO<CommandePrioritaire> {
	...
	private final static String GET_COMMANDE_PRIORITAIRE= "SELECT TOP 1 *"
			+"FROM Commandes c"
			+"WHERE c.statut = 'A TRAITER"
			+"ORDER BY c.date;";
	
	private final static String GET_DETAIL_COMMANDE= "SELECT *"
			+"FROM Commandes c"
			+"LEFT JOIN Societes s ON c.idSociete = s.idSociete"
			+"LEFT JOIN LignesCommandes lc ON c.idCommande = lc.idCommande"
			+"INNER JOIN Articles a ON  lc.idArticle= a.id"
			+ "WHERE c.idCommande=? ";
